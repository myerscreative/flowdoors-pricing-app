...
import {
  ...
  updateQuoteSalesRep,
  moveQuotePipeline,
  // NOTE: if you have updateQuoteTags available, you can add it to the InlinePipeline props later
...
import QuotesGrid from '@/components/admin/QuotesGrid'
...
/**
 * If you already created a shared util like `@/utils/quote-utils`,
 * prefer importing:  import { getStatusClasses } from "@/utils/quote-utils";
 * For now, keep a local definition so this page compiles on its own.
 */
function getStatusClasses(status: string) {
  switch (status) {
    ...
    case 'Qualified':
      return {
        buttonColorClass: 'bg-blue-300',
        customClass: 'bg-blue-100 text-blue-800 border-blue-300',
      }
    case 'Quoted':
      return {
        buttonColorClass: 'bg-indigo-300',
    ...
    case 'Negotiation':
      return {
        buttonColorClass: 'bg-orange-300',
        customClass: 'bg-orange-100 text-orange-800 border-orange-300',
      }
    case 'Closed Won':
      return {
        buttonColorClass: 'bg-green-400',
        customClass: 'bg-green-100 text-green-800 border-green-300',
      }
    case 'Closed Lost':
      return {
        buttonColorClass: 'bg-red-300',
...
/** Fields that may show up from multiple quote shapes */
type LooseQuote = Quote & {
  quoteNumber?: string
  quoteAmount?: number
  salesRep?: string // may be a name or an id token
...
/** Normalize possible rep IDs */
const normalizeRepId = (s: string): string => s.replace(/^PS-/i, 'SP-')

/** Safely resolve items count without `any` */
const safeItemsCount = (q: unknown): number => {
...
export default function AdminQuotesPage() {
  ...
  const isMinimized = prefs.view === 'cards'
  ...
  // Resolve sales rep *name* consistently from various shapes
  const resolveSalesRepName = useCallback(
    (quote: LooseQuote): string => {
      ...
      // If it's already a human name (not an ID-ish token), return it as-is
      if (
        typeof raw === 'string' &&
        !raw.startsWith('SP-') &&
        !raw.startsWith('PS-') &&
      ...
      const normalized = normalizeRepId(id)
      const hit =
      ...
      if (isDebug) {
        ...
        console.warn(
          ...
          fetchedQuotes.slice(0, 3).map((q) => {
            const lq = q as unknown as LooseQuote
            return {
              id: q.id,
              salesperson_id: lq.salesperson_id,
              salesRep: lq.salesRep,
              last_modified_by: lq.last_modified_by,
              resolvedName: resolveSalesRepName(lq),
            }
          })
        )
      }
  ...
  const handleUpdateFollowUp = async (_quoteId: string, _newDate: Date) => {
    // intentionally empty
  }
    ...
    const dataLines = rows
      .map((q) => {
        ...
        return [
          ...
          lq.numberOfItems ?? safeItemsCount(lq),
          followISO,
        ]
          .map(escapeCsv)
          .join(',')
    ...
    <main className="flex-1 p-6 md:p-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Manage and view all customer quotes.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            className="h-9 rounded-lg"
            onClick={() => setPrefs({ view: isMinimized ? 'table' : 'cards' })}
      ...
      <div className="flex flex-wrap items-center gap-3">
        {(STATUS_FILTERS ?? []).map((status: Status) => {
          ...
          return (
            <button
              ...
              className={cn(
                'flex-shrink-0 w-24 h-10 rounded-lg shadow-sm overflow-hidden transform transition-transform duration-150',
                'focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary',
                filterMode === 'status' && activeStatus === status
                  ? 'ring-2 ring-primary ring-offset-2 scale-105'
                  : 'ring-0'
              )}
            >
              <div className="h-4/6 bg-white flex items-center justify-center">
                <span className="font-semibold text-sm text-gray-800">
                  {status}
                </span>
              </div>
              <div className={cn('h-2/6', buttonColorClass)} />
            </button>
          )
        })}
        <Button
          size="sm"
          className={cn(
            'h-10 w-24 transform transition-transform duration-150',
        ...
        <Button
          ...
          size="sm"
          className={cn(
            'h-10 w-auto px-4 gap-2 transform transition-transform duration-150 relative',
            filterMode === 'dueToday'
              ? 'ring-2 ring-primary ring-offset-2 scale-105'
              : 'ring-0'
          )}
          ...
          <div>
            {(grouped[stage] || []).map((q) => {
              ...
              return (
                ...
                >
                  {/* Header: status pill + created date */}
                    ...
                    <div>
                      ...
                      <span className="font-medium">
                        {(() => {
                          ...
                          const hit =
                            salespeople.find(
                              (sp) => sp.salesperson_id === normalized
                            ) || salespeople.find((sp) => sp.id === normalized)
                          return hit?.name ?? id
                        })()}
                      </span>
                  ...
                  {isExpanded && (
                    <div className="rounded-lg bg-gray-50/80 border border-gray-100 p-3 text-sm text-gray-800 space-y-2 mb-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span>{lq.numberOfItems ?? safeItemsCount(lq)}</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between">
                    <button
                      className="text-[11px] text-gray-400 cursor-not-allowed"
                      disabled
                      title="Tag editing not available in inline view"
                    >
                      Notes · Tasks{' '}
